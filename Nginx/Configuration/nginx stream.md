...
- **Core:** The Nginx stream module proxies TCP and UDP at OSI layer 4: it does not inter…
...
- `ngx_stream_js_module` adds JS hooks for stream logic (optional
...
## Mistakes to Avoid
- **Mistake:** Putting HTTP directives inside `stream {}`
- **Mistake:** Expecting active HTTP health checks from OSS stream
- **Mistake:** Forgetting custom stream access_log formats when debugging opaqu…
...
- vs [[nginx using unix socket]]: unix sockets are a local upstream transport


### Use cases
- TCP proxy Postgres through a bastion Nginx

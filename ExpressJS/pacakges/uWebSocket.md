...
- **Core:** `uWebSockets.js` exposes `uWS.App` / `uWS.SSLApp` with HTTP and WebSocket rou…
...
## Mistakes to Avoid
- **Mistake:** Expecting Express middleware to mount unchanged
- **Mistake:** Ignoring backpressure until memory blows up
- **Mistake:** Assuming install works on every CI image without native build su…
- **Mistake:** Choosing µWS for a CRUD app that needs the Express ecosystem
...
## Comparison
- vs [[Socket IO]]: Socket.IO adds rooms, fallbacks, and adapters
...
### Use cases
- High-fanout tickers, game relays, and chat backends where Express+`ws` CPU or…

- **Example:** A market-data feed buffers gigabytes when clients stall

...
## Key Concepts
- **Path-vector protocol:** each advertisement carries AS path, next hop, and attributes → loops are dete…
- **Policy over pure distance:** unlike OSPF/IS-IS, BGP optimizes who may use which paths → LOCAL_PREF, commun…
- **eBGP vs iBGP:** eBGP peers across [[autonomous system]] boundaries
- **Prefix + ASN:** you advertise [[CIDR (Classless Inter-Domain Routing)]] blocks with an autono…
...
- Host-level "BGP broken" symptoms are usually upstream: prefix hijacks, leaked…

## Mistakes to Avoid
- **Mistake:** Debugging "BGP" on a single host's default gateway
- **Mistake:** Advertising routes without filters — classic cause of route leaks
- **Mistake:** Confusing shorter AS_PATH with "always best"
- **Mistake:** Ignoring more-specific prefixes
...
## Comparison
- vs [[Internal routing]] (OSPF/IS-IS): interior protocols optimize metrics ins…


### Use cases
- ISP peering, multi-homed enterprise edge, and anycast announcements. Example:…

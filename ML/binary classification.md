...
## Key Concepts
- **Note:** Output is usually **P(y=1 | x)** ∈ [0,1] from [[sigmoid]] (logistic) or margi…

```txt
- **Note:** score(x) → probability p → if p ≥ τ then positive else negative
```

- **Note:** Imbalanced data (1% fraud): 99% accuracy by predicting all negatives

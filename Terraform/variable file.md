...
## Mistakes to Avoid
- **Mistake:** Committing secrets in tfvars
- **Mistake:** Debugging “wrong value” without checking precedence
- **Mistake:** Expecting child modules to see root variables without passing th…
- **Mistake:** Treating `sensitive` as encryption
...
### Use cases
- Per-env `prod.tfvars` / `dev.tfvars`, module inputs for VPC CIDRs, and output…

- **Example:** CI injects `TF_VAR_db_password` from a secret store while `envir…

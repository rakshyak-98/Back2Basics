...
- **Master vs workers:** Master binds ports, reads configuration, manages workers
- **HTTP phases:** post-read → server rewrite → find configuration → rewrite → pre-access → acce…
- **Upstream subsystem:** Connect, retry, load balance, keepalive pool to backends.


- **Core:** Nginx internals are a master/worker process model plus an event module ([[Epo…
...
## Mistakes to Avoid
- **Mistake:** Using `if` as general programming in `location`
- **Mistake:** Ignoring unix socket backlog
- **Mistake:** Leaving `proxy_buffering` on for SSE/event streams
- **Mistake:** Duplicate upstream names across includes — last definition wins
- **Mistake:** Raising `worker_connections` without `worker_rlimit_nofile` and …
...
### Use cases
- Tune workers and upstream keepalive for a busy API gateway

...
- **Default route:** `/etc/asound.conf` or `~/.asoundrc` picks the default card when multiple outp…
- **Userspace stack:** PipeWire/PulseAudio sit above ALSA; fixing sinks often means both layers.


- **Core:** ALSA provides drivers, the mixer API, and PCM I/O
...
## Mistakes to Avoid
- **Mistake:** Editing only `alsamixer` while PipeWire still routes to a muted …
- **Mistake:** Assuming one “sound card”
- **Mistake:** Leaving a process holding `/dev/snd/*` and blaming the driver fo…
...
## Comparison
- vs PipeWire/PulseAudio: ALSA is the device driver layer


### Use cases
- After docking a laptop to an HDMI monitor, playback stays on the laptop speak…

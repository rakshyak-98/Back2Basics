...
"""
	\\; end of the command
	{} place holder for the directory path
"""
```

[user management](https://www.notion.so/user-management-fcce2450e15c4c1aa8b0ebfad6dd3e25?pvs=21)

- `\\;`: This signifies the end of the `-exec` command and is necessary to terminate the `-exec` option.

uid permission bits : setuid (SUID), setguid (SGID) bits. associated with the user and group ownership of a file or directory.

> cannot change it using the `chmod` command because sockets are a special type of file that doesn't have the execute permission or the ability to set the setuid or setgid bits.
